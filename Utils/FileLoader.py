...
class FileLoader():
    ...
    # Object
    def load_object_from_disk(self, path, object_name=None):
        """
        Load an object from the specified file path using pickle. Returns the deserialized object.
        """
        with open(path, "rb") as f:
            stored_object = pickle.load(f)
        self.logger.LogMessage(f"Object {object_name if object_name else ''} loaded from {path}")
        return stored_object

    def store_object_on_disk(self,object_to_store, path, object_name=None):
        """
        store an object to the specified file path using pickle.
        """
        with open(path, "wb") as f:
            pickle.dump(object_to_store, f)
        self.logger.LogMessage(f"Object {object_name if object_name else ''} saved to {path}")
    ...
    def load_documents_from_disk(self, file_path:str) -> list[Document]:
        ...
        """
        serializable_docs = self.load_json_from_disk(file_path, "Documents")    
    ...
    def store_documents_on_disk(self, documents:list[Document], file_path: str) -> None:
        ...
        self.store_json_on_disk(serializable_docs, file_path, "Documents")  
    ...
    def store_raw_ingredient_lists_on_disk(self, obj_list: list[RawIngredientList], file_path: str):   
        ...
        self.store_json_on_disk(dict_list, file_path, "RawIngredientLists")    
    ...
    def load_raw_ingredient_lists_from_disk(self,file_path: str) -> list[RawIngredientList]:  
        ...
        """  
        data = self.load_json_from_disk(file_path, "RawIngredientLists")    
    ...
    def store_complex_ingredient_list_on_disk(self, obj_list: list[ComplexIngredientList], file_path: str) -> None:    
        ...
        """
        dict_list = [obj.model_dump() for obj in obj_list]        
        self.store_json_on_disk(dict_list, file_path, "ComplexIngredientLists")    
    ...
    def load_complex_ingredient_list_from_disk(self, file_path: str) -> list[ComplexIngredientList]:
        ...
        """
        data = self.load_json_from_disk(file_path, "ComplexIngredientLists")  
    ...
    # Json
    def store_json_on_disk(self, obj, file_path, object_name: str = None):
        with open(file_path, "w", encoding="utf-8") as f:
            json.dump(obj, f, ensure_ascii=False)
        self.logger.LogMessage(f"Json object {object_name if object_name else ''} stored to {file_path}")
        
    def load_json_from_disk(self, file_path, object_name: str = None):
        with open(file_path, "r", encoding="utf-8") as f:
            data = json.load(f)
        self.logger.LogMessage(f"Json object {object_name if object_name else ''} loaded from {file_path}")
        return data
